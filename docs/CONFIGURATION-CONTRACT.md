...
### Internal wallet staging controls
`INTERNAL_WALLET_USER_TOPUP_ENABLED`, `INTERNAL_WALLET_ADMIN_CREDIT_ENABLED`. User top-up is a non-production staging capability and remains disabled in production regardless of this flag; admin sandbox credit is also fail-closed in production unless explicitly enabled by deployment policy.

### Payments
`PAYMENT_PROVIDER`, `PAYMENT_CURRENCY`, `PAYMENT_WEBHOOK_SECRET`, `PAYMENT_PROVIDER_TOKEN`, `PAYMENT_PROVIDER_CREATE_URL`, `PAYMENT_PROVIDER_RELEASE_URL`, `PAYMENT_PROVIDER_REFUND_URL`, `PAYMENT_PROVIDER_PAYOUT_STATUS_URL`, `PAYMENT_PROVIDER_TIMEOUT_MS`, `PAYMENT_PROVIDER_MAX_ATTEMPTS`, `PAYMENT_PROVIDER_RETRY_BASE_MS`, `PAYMENT_WEBHOOK_MAX_AGE_SECONDS`. Production accepts `internal` or `webhook`; `internal` requires `PAYMENT_CURRENCY=TOMAN` and does not require external PSP credentials/endpoints.
...
Never commit real secret values. Production must reject placeholder authentication secrets, missing metrics token, missing payment/reset secrets, wildcard CORS, non-HTTPS external endpoints, non-PostgreSQL persistence, local storage, and simulator payments. External PSP credentials/endpoints are required only when `PAYMENT_PROVIDER=webhook`; internal ledger mode is explicitly allowed for the temporary non-bank-money phase.
...
## Internal wallet / temporary payment mode

When `PAYMENT_PROVIDER=internal` and `PAYMENT_CURRENCY=TOMAN`, payment funding and release use the PostgreSQL internal wallet ledger. `GET /api/v1/wallet/me` exposes the authenticated user's internal balance; `GET /api/v1/wallet/transactions` exposes wallet transaction history. `POST /api/v1/wallet/transfer` supports internal currency transfers with an idempotency key. `POST /api/v1/wallet/top-up` is a user-scoped internal-provider staging operation and is disabled in production; it requires `INTERNAL_WALLET_USER_TOPUP_ENABLED=true`. `POST /api/v1/wallet/sandbox-credit` remains admin-only for controlled test funding.
