...
cd "$ROOT"
export ACCESS_TOKEN_SECRET="${ACCESS_TOKEN_SECRET:-$(openssl rand -hex 32)}"
export REFRESH_TOKEN_SECRET="${REFRESH_TOKEN_SECRET:-$(openssl rand -hex 32)}"
export METRICS_TOKEN="${METRICS_TOKEN:-$(openssl rand -hex 24)}"
export PAYMENT_PROVIDER_TOKEN="${PAYMENT_PROVIDER_TOKEN:-$(openssl rand -hex 24)}"
...
export PAYMENT_WEBHOOK_SECRET="${PAYMENT_WEBHOOK_SECRET:-$(openssl rand -hex 32)}"
...
export NOTIFICATION_PROVIDER_TOKEN="${NOTIFICATION_PROVIDER_TOKEN:-$(openssl rand -hex 24)}"
export RESET_TOKEN_DELIVERY_URL="${RESET_TOKEN_DELIVERY_URL:-https://reset.invalid/deliver}"
export RESET_TOKEN_DELIVERY_SECRET="${RESET_TOKEN_DELIVERY_SECRET:-$(openssl rand -hex 32)}"
