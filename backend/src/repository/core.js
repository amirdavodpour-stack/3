...
export async function findJobApplication(jobId,candidateId) { const {rows}=await requirePool().query(`SELECT * FROM job_applications WHERE job_id=$1 AND candidate_id=$2 AND status IN ('PENDING','SHORTLISTED') LIMIT 1`,[jobId,candidateId]); return rows[0]?applicationFromRow(rows[0]):null; }
...
export async function listOffersForProvider(providerId) {
  const { rows } = await requirePool().query(
    `SELECT * FROM offers WHERE provider_id=$1 ORDER BY created_at DESC`,
    [providerId],
  );
  return rows.map(offerFromRow);
}
