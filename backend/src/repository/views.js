...
export async function listOfferViewsForJob(jobId) {
  ...
  return rows.map(o=>({
    id:o.id,jobId:o.job_id,providerId:o.provider_id,price:String(o.price ?? '0'),message:o.message,status:o.status,
...
export async function listLedgerEntriesForPayment(paymentId) {
  ...
  return rows.map(r=>({id:r.id,journalId:r.journal_id,referenceType:r.reference_type,referenceId:r.reference_id,account:r.account,debit:(String(r.currency || 'USD').toUpperCase() === 'TOMAN' ? String(r.debit ?? '0') : Number(r.debit||0)),credit:(String(r.currency || 'USD').toUpperCase() === 'TOMAN' ? String(r.credit ?? '0') : Number(r.credit||0)),currency:r.currency,createdAt:r.created_at?.toISOString?.() ?? r.created_at}));
}
