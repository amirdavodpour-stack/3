...
import { createWalletRoutes } from './application/wallet_routes.js';
...
async function recommendedJobs(req, res) {
  ...
  const visibility = url.searchParams.get('visibility') ? enumField(url.searchParams.get('visibility'), JOB_VISIBILITY, 'visibility') : null;
  const search = url.searchParams.get('q') ? textField(url.searchParams.get('q'), 'q', {min:1,max:120}) : null;
  ...
  const rows = process.env.DATABASE_URL
    ...
    : appLegacy.jobs().filter(j =>
        j.status === 'PUBLISHED' &&
        (!kind || (j.kind || (j.jobType==='FIXED'?'MISSION':'JOB')) === kind) &&
        (!visibility || String(j.visibility || 'PUBLIC').toUpperCase() === visibility) &&
        (!categoryId || String(j.categoryId) === categoryId || categoryBy(categoryId)?.id === j.categoryId) &&
        (!search || [j.title, j.description, j.city, j.categoryId]
          .filter(Boolean)
          .some(value => String(value).toLowerCase().includes(search.toLowerCase())))
      ).map(job => ({job,category:categoryBy(job.categoryId)?.name,owner:null,provider:null,offerCount:0}));
  const scored = [];
