...
export async function handleCreate(ctx, req, res, parts, user) {
  const {
    ...
    textField, moneyField, tomanField, enumField, JOB_TYPES, JOB_KINDS, JOB_VISIBILITY,
  ...
  const monthlySalary = body.monthlySalary == null || body.monthlySalary === '' ? null : tomanField(body.monthlySalary, 'monthlySalary');
  ...
  const budgetMin = tomanField(body.budgetMin, 'budgetMin');
  const budgetMax = tomanField(body.budgetMax, 'budgetMax');
