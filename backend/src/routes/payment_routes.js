...
export function createPaymentRoutes({
  ...
  if (req.method === 'POST' && parts[1] === 'fund' && parts[2]) {
    ...
    enforceJobState(job, existing?.status === 'HOLD_FAILED' ? ['PUBLISHED', 'ASSIGNED', 'FUNDED'] : ['PUBLISHED', 'ASSIGNED']);
    let providerId = job.providerId || (existing?.status === 'HOLD_FAILED' ? existing.payeeId : null);
    let chosenOfferId = null;
    if (!providerId) {
      const chosen = process.env.DATABASE_URL ? (await repo.listOffersForJob(job.id)).filter((o)=>o.status==='PENDING').sort((a,b)=>{ const aa=BigInt(String(a.price||'0')); const bb=BigInt(String(b.price||'0')); return aa<bb?-1:aa>bb?1:0; })[0] : paymentLegacy.choosePendingOffer(job.id);
