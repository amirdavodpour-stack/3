...
export const MAX_TOMAN_UNITS = 9_000_000_000_000_000n;

export function tomanField(value, field) {
  const raw = String(value ?? '').trim();
  if (!/^\d+$/.test(raw)) {
    throw new HttpError(400, 'INVALID_AMOUNT', `${field} must be a positive integer TOMAN amount`);
  }
  try {
    const amount = BigInt(raw);
    if (amount <= 0n || amount > MAX_TOMAN_UNITS) throw new Error('range');
    return amount.toString();
  } catch {
    throw new HttpError(400, 'INVALID_AMOUNT', `${field} must be between 1 and 9000000000000000 TOMAN`);
  }
}
