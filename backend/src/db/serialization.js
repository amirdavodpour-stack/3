...
export const tableColumns = {
  ...
  refunds: ['payment_id','amount','currency','status','provider_ref','idempotency_key','created_at','updated_at'],
...
export function toDbRow(collection, row) {
  switch (collection) {
    ...
    case 'ledgerEntries': { const currency=String(row.currency || config.paymentCurrency).toUpperCase(); return [row.id,row.journalId,row.referenceType,row.referenceId,row.account,currency==='TOMAN'?String(row.debit ?? '0'):Number(row.debit||0),currency==='TOMAN'?String(row.credit ?? '0'):Number(row.credit||0),currency,row.createdAt]; }
    ...
    case 'refunds': return [row.id,row.paymentId,row.amount,row.currency || config.paymentCurrency,row.status,row.providerRef || null,row.idempotencyKey || null,row.createdAt,row.updatedAt];
...
function financialValue(currency, value) {
  return String(currency || config.paymentCurrency).toUpperCase() === 'TOMAN'
    ? String(value ?? '0')
    : Number(value ?? 0);
}
...
export function fromDbRow(collection, r) {
  switch (collection) {
    ...
    case 'jobs': return { id:r.id,ownerId:r.owner_id,providerId:r.provider_id,title:r.title,description:r.description,categoryId:r.category_id,jobType:r.job_type,budgetType:r.budget_type,budgetMin:String(r.budget_min ?? '0'),budgetMax:String(r.budget_max ?? '0'),duration:r.duration,acceptanceCriteria:r.acceptance_criteria,status:r.status,city:r.city,kind:r.kind || (r.job_type==='FIXED'?'MISSION':'JOB'),visibility:r.visibility || 'PUBLIC',schedule:r.schedule || null,monthlySalary:r.monthly_salary==null?null:String(r.monthly_salary),applicationDeadline:r.application_deadline ? (r.application_deadline.toISOString?.() ?? r.application_deadline) : null,createdAt:r.created_at?.toISOString?.() ?? r.created_at,updatedAt:r.updated_at?.toISOString?.() ?? r.updated_at,publishedAt:r.published_at ? (r.published_at.toISOString?.() ?? r.published_at) : null };
    case 'offers': return { id:r.id,jobId:r.job_id,providerId:r.provider_id,price:String(r.price ?? '0'),message:r.message,status:r.status,createdAt:r.created_at?.toISOString?.() ?? r.created_at,updatedAt:r.updated_at?.toISOString?.() ?? r.updated_at };
    ...
    case 'payments': return { id:r.id,jobId:r.job_id,payerId:r.payer_id,payeeId:r.payee_id,amount:financialValue(r.currency, r.amount),status:r.status,providerRef:r.provider_ref,idempotencyKey:r.idempotency_key || '',baseAmount:financialValue(r.currency, r.base_amount ?? r.amount),employerFee:financialValue(r.currency, r.employer_fee || 0),workerFee:financialValue(r.currency, r.worker_fee || 0),platformFee:financialValue(r.currency, r.platform_fee || 0),employerCharge:financialValue(r.currency, r.employer_charge ?? r.amount),providerPayout:financialValue(r.currency, r.provider_payout ?? r.amount),feePolicyVersion:r.fee_policy_version || 'legacy',currency:r.currency || config.paymentCurrency,createdAt:r.created_at?.toISOString?.() ?? r.created_at,updatedAt:r.updated_at?.toISOString?.() ?? r.updated_at };
    case 'ledgerEntries': return { id:r.id,journalId:r.journal_id,referenceType:r.reference_type,referenceId:r.reference_id,account:r.account,debit:financialValue(r.currency, r.debit||0),credit:financialValue(r.currency, r.credit||0),currency:r.currency || config.paymentCurrency,createdAt:r.created_at?.toISOString?.() ?? r.created_at };
    case 'settlements': return { id:r.id,paymentId:r.payment_id,providerRef:r.provider_ref,amount:financialValue(r.currency, r.amount),currency:r.currency || config.paymentCurrency,status:r.status,createdAt:r.created_at?.toISOString?.() ?? r.created_at,updatedAt:r.updated_at?.toISOString?.() ?? r.updated_at };
    case 'refunds': return { id:r.id,paymentId:r.payment_id,amount:financialValue(r.currency, r.amount),currency:r.currency || config.paymentCurrency,status:r.status,providerRef:r.provider_ref,idempotencyKey:r.idempotency_key || '',createdAt:r.created_at?.toISOString?.() ?? r.created_at,updatedAt:r.updated_at?.toISOString?.() ?? r.updated_at };
