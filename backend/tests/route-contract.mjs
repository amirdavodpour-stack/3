...
test('offer read APIs are authenticated and viewer-scoped', async () => {
  const source = await readFile(new URL('../src/routes/offer_routes.js', import.meta.url), 'utf8');
  assert.match(source, /req\.method === 'GET' && parts\.length === 1/);
  assert.match(source, /jobId query parameter is required/);
  assert.match(source, /job\.ownerId !== me\.id/);
