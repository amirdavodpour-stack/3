...
test('full marketplace flow', async () => {
  let r=await json('/auth/register',{method:'POST',body:JSON.stringify({email:'owner@example.com',password:'pass123456789',displayName:'Owner'})}); assert.equal(r.status,201); alice=r.body.data;
  r=await json('/auth/register',{method:'POST',body:JSON.stringify({email:'provider@example.com',password:'pass123456789',displayName:'Provider'})}); assert.equal(r.status,201); bob=r.body.data;
