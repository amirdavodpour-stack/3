...
Production payment configuration requires an explicit provider boundary, for example `PAYMENT_PROVIDER=webhook`; internal wallet mode requires `PAYMENT_PROVIDER=internal` with `PAYMENT_CURRENCY=TOMAN`.
...
`PAYMENT_PROVIDER=internal` is the temporary production-safe mode when no external
PSP is configured. It uses the PostgreSQL-backed internal `HOPE` wallet/ledger and
does not move bank money. Set `PAYMENT_CURRENCY=TOMAN`. The existing `webhook`
adapter remains available for a real or sandbox PSP once its HTTPS facade exists.
`simulator` remains for isolated local/test use only. The provider boundary is
documented in `src/payment_provider.js`; external payment and bank cash-out can
be attached later without replacing the internal ledger.
