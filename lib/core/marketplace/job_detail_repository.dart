...
abstract interface class JobDetailRepository {
  ...
  Future<Map<String, dynamic>> compareCandidates(String jobId, List<String> applicationIds);
  Future<void> reportJob(String jobId, {required String reason, String details = ''});
}
...
class ApiJobDetailRepository implements JobDetailRepository {
  ...
  @override
  Future<Map<String, dynamic>> compareCandidates(String jobId, List<String> applicationIds) async =>
      Map<String, dynamic>.from(await _api.request(
        'POST',
        '/jobs/${Uri.encodeComponent(jobId)}/candidates/${Uri.encodeComponent(applicationIds.first)}/compare',
        auth: true,
        body: {'applicationIds': applicationIds.take(5).toList()},
      ) as Map);
  ...
  Future<void> reportJob(String jobId, {required String reason, String details = ''}) =>
      _api.request(
        'POST',
        '/jobs/${Uri.encodeComponent(jobId)}/report',
        auth: true,
        body: {'reason': reason.trim(), 'details': details.trim()},
      ).then((_) {});
}
