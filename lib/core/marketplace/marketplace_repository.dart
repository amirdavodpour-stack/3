...
class OfflineMarketplaceRepository implements MarketplaceRepository {
  ...
  Future<HopeJob> getOpportunity(String id) async {
    try {
      ...
      // Detail responses can contain owner/provider-specific fields when the
      // authenticated caller is allowed to see a private opportunity. Never
      // persist such a response into the device-wide SharedPreferences cache:
      // another account using the same device must not inherit private data.
      final publiclyCacheable = job.status?.toUpperCase() == 'PUBLISHED' &&
          job.visibility.toUpperCase() == 'PUBLIC';
      if (publiclyCacheable) {
        await _writeJobs(_key('opportunity', id), [job]);
      } else {
        await _removeCachedKey(_key('opportunity', id));
      }
      return job;
    } catch (error) {
