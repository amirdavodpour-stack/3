...
CreateJobValidationResult validateCreateJob({
  ...
  final minRaw = minBudget.trim();
  final maxRaw = maxBudget.trim();
  final amountPattern = RegExp(r'^\d+$');
  if (!amountPattern.hasMatch(minRaw)) {
    return const CreateJobValidationResult(error: 'حداقل بودجه باید یک عدد صحیح تومان باشد.');
  }
  if (!amountPattern.hasMatch(maxRaw)) {
    return const CreateJobValidationResult(error: 'حداکثر بودجه باید یک عدد صحیح تومان باشد.');
  }
  final min = BigInt.tryParse(minRaw);
  final max = BigInt.tryParse(maxRaw);
  const maxToman = 9000000000000000;
  ...
  if (min == null || min <= BigInt.zero || min > BigInt.from(maxToman)) {
    return const CreateJobValidationResult(error: 'حداقل بودجه معتبر نیست.');
  }
  if (max == null || max <= BigInt.zero || max > BigInt.from(maxToman)) {
