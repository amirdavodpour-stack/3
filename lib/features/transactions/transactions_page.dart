...
class _TransactionsPageState extends State<TransactionsPage> {
  ...
  void didChangeDependencies() {
    ...
    if (id != loadedUserId) {
      ...
      future = id == null ? null : registry.listMyJobs();
  ...
  Future<void> reload() async {
    if (!mounted) return;
    final next = applicationRegistryOf(context).listMyJobs();
    _paymentFutures.clear();
  ...
  Future<HopePayment?> _tryGetPayment(String jobId) {
    return _paymentFutures.putIfAbsent(jobId, () async {
      try {
        return await applicationRegistryOf(context).loadTransaction(jobId);
      } catch (_) {
