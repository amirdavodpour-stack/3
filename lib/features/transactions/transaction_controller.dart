...
class TransactionController {
  ...
  Future<HopePayment> execute(String operation) => _commandUseCase(
        jobId,
        operation,
        idempotencyKey: operation == 'fund' ? _fundIdempotencyKey : null,
      );

  Future<HopeJob> executeJob(String operation) {
    switch (operation) {
      case 'start':
        return repository.startJob(jobId);
      case 'deliver':
        return repository.deliverJob(jobId);
      case 'accept':
        return repository.acceptJob(jobId);
      default:
        throw StateError('Unsupported job operation: $operation');
    }
  }
}
